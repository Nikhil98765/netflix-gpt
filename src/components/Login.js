...
const Login = ({ onSignUp }) => {
  ...
  return (
    <div>
      ...
      <div>
        <img
          className="absolute object-cover h-screen md:h-auto"
          src={NETFLIX_BG_IMG}
          alt="Netflix Logo"
        />
      </div>
      <form
        ...
        className="absolute p-6 md:p-12 bg-black w-[80%] md:w-3/12 my-36 mx-auto left-0 right-0 text-white rounded-lg bg-opacity-80">
        ...
        <input
          ...
          className="p-3 md:p-4 my-2 md:my-4 w-full bg-gray-700 rounded-lg"
        />
        <button className="p-3 md:p-4 my-4 md:my-6 bg-red-700 w-full rounded-lg">
          {isSignIn ? 'Sign In' : 'Sign Up'}
        </button>
        <p className="text-red-700 font-bold text-lg py-2">{errorMessage}</p>
        <p className="py-1 md:py-4 cursor-pointer" onClick={toggleSignInForm}>
